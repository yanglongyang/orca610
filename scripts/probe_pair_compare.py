...
PROTOCOL_KEYS = (
    ...
    "solvent_regime", "cpcmeq", "tda", "excited_state_method", "absorption_protocol",
    "emission_protocol", "numerical_settings",
)
...
def protocol_issues(first: dict, second: dict) -> list[str]:
    """Return all unmatched or missing protocol fields for a species pair."""
    issues: list[str] = []
    for key in PROTOCOL_KEYS:
        a, b = first.get(key), second.get(key)
        if _missing(a) or _missing(b):
            issues.append(f"missing protocol field {key!r}")
        elif a != b:
            issues.append(f"{key}: {a!r} != {b!r}")
    return issues
